...
func ConsumeGroup(client *cluster.Client, topics []string, ch chan *ConsumerData) {
	...
	for msg := range consumer.Messages() {
		...
		log.WithFields(log.Fields{"topics": topics, "group": group, "partition": msg.Partition, "offset": msg.Offset, "key": data.Key, "value size": len(data.Value), "rid": rid, "code": "req-started"}).Info("New Group Message Received")
		ch <- &data
