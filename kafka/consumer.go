...
func Consume(client *sarama.Client, topic string, ch chan *ConsumerData, options ...ConsumerOptions) {
	...
	for msg := range pc.Messages() {
		...
		log.WithFields(log.Fields{"topic": topic, "partition": msg.Partition, "offset": msg.Offset, "key": data.Key, "value size": len(data.Value), "rid": data.Key, "code": "reply-received"}).Info("New Message Received")
		ch <- &data
		<-data.Processed
	}
